...
def get_dashboard_data(session: requests.Session) -> tuple[dict | None, str | None]:
    ...
    try:
        ...
        if not email:
            email_match_fallback = re.search(r'<div id="mectrl_currentAccount_secondary"[^>]*>\s*([^<]+)\s*</div>', html_text)
            email = email_match_fallback.group(1).strip() if email_match_fallback else "未知邮箱"
...
def bing_search(session: requests.Session, query_str: str, market: str):
    """执行单次电脑端 Bing 搜索。"""
    try:
        bing_domain = "https://cn.bing.com" if market == "zh-CN" else "https://www.bing.com"
        url = f'{bing_domain}/search?q={query_str}&form=TSASDS'
        headers = {
        ...
        if response.status_code != 200:
            print_log('搜索失败', f"关键词: {urllib.parse.unquote(query_str)}, 状态码: {response.status_code}")
    except Exception as e:
        print_log('搜索异常', str(e))

def complete_daily_set_tasks(session: requests.Session, dashboard_data: dict, request_token: str):
    """完成每日任务集。"""
    ...
    incomplete_tasks = [t for t in todays_tasks if not t.get('complete') and 'name' in t and 'hash' in t]

    if not incomplete_tasks:
        print_log('任务总结', "今日所有每日任务均已完成。")
        return

    print_log('任务信息', f"发现 {len(incomplete_tasks)} 个未完成的每日任务，即将执行。")
    ...
    for i, task in enumerate(incomplete_tasks, 1):
        ...
        try:
            print(f"({i}/{len(incomplete_tasks)}) 正在处理: '{task.get('title', '未知任务')}'...", end='', flush=True)
            response = session.post(post_url, headers=post_headers, data=payload, timeout=15)
            time.sleep(random.uniform(2, 4))
            if response.ok and response.json().get('status') == 'Success':
                result = response.json()
                print(f" 成功! 当前总积分: {result.get('balance', 'N/A')}。")
            else:
                print(f" 失败! 状态码: {response.status_code}, 原因: {response.text[:100]}")
        except Exception as e:
            print(f" 出错! 异常: {e}")
...
def start_main(bing_ck: str) -> str | None:
    ...
    pc_progress, pc_total = pc_search.get('pointProgress', 0), pc_search.get('pointProgressMax', 0)
    ...
    if pc_progress < pc_total:
        ...
        for i in range(1, search_count + 1):
            ...
            bing_search(session, query, market)
            time.sleep(random.uniform(2.5, 4.5))

            if i % 3 == 0:
                new_dashboard, _ = get_dashboard_data(session)
                if new_dashboard:
                    new_balance = new_dashboard.get('userStatus', {}).get('availablePoints', old_balance)
                    print_log(f"电脑搜索第{i}次后", f"当前积分: {new_balance} (较上次增加: {new_balance - old_balance})")
    ...
    if final_dashboard:
        final_balance = final_dashboard.get('userStatus', {}).get('availablePoints', start_balance)
        points_earned = final_balance - start_balance
        print_log("本轮总结", f"初始: {start_balance}, 最终: {final_balance}, 获得: {points_earned}")
        ...
        try:
            ...
            today_str = datetime.date.today().strftime('%m/%d/%Y')
            daily_promos = final_dashboard.get('dailySetPromotions', {}).get(today_str, [])
            if daily_promos:
            ...
            print_log('今日总计', f"已获得 {total_daily_points_earned} 积分 (基于当前状态)")
...
if __name__ == '__main__':
    ...
    if not BING_COOKIE or "请在这里粘贴" in BING_COOKIE:
        print_log("错误", "未配置有效 Cookie。请设置 BING_COOKIE 环境变量或在脚本中修改。")
    else:
        all_cookies = [ck.strip() for ck in BING_COOKIE.strip().split("\n") if ck.strip()]
